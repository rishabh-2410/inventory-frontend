      ...
      >
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center">
              <View className="mr-4 h-12 w-12 items-center justify-center rounded-full border border-[#0b7a4d] bg-[#dff1ea]">
                <Text className="text-[18px] font-inter-semibold text-[#0b7a4d]">IP</Text>
              </View>
              <View>
                <Text className="text-[15px] font-inter-medium text-[#8a94a6]">Welcome back</Text>
                <Text className="text-[18px] font-inter-bold text-[#171a21]">{user ? user.name : "Guest"}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View className="px-6 pt-6">
          {isDashboardStatsError ? (
            <Text className="text-[16px] font-inter-regular mt-4 text-[#7a8596]">Error loading dashboard stats</Text>
          ) : (
            <>
            ...
            <View className="h-12 w-12 items-center justify-center rounded-2xl bg-[#dff1ea]">
              <Text className="text-[20px] font-inter-semibold text-[#159b67]">$</Text>
            </View>
            <Text className="mt-4 text-[16px] font-inter-medium text-[#7a8596]">Current Inventory</Text>
            <Text className="mt-2 text-[24px] font-inter-bold text-[#171a21]">
          ...
          <View className="mb-5 rounded-[22px] border border-[#e5e7eb] bg-white px-5 py-5 shadow-sm">
            <View className="h-12 w-12 items-center justify-center rounded-2xl bg-[#e4efff]">
              <Text className="text-[20px] font-inter-semibold text-[#2563eb]">H</Text>
            </View>
            <Text className="mt-4 text-[16px] font-inter-medium text-[#7a8596]">Low Stock Products</Text>
            <Text className="mt-2 text-[24px] font-inter-bold text-[#171a21]">
          ...
          <View className="rounded-[22px] border border-[#e5e7eb] bg-white px-5 py-5 shadow-sm">
            <View className="h-12 w-12 items-center justify-center rounded-2xl bg-[#fce8e8]">
              <Text className="text-[20px] font-inter-semibold text-[#b54343]">T</Text>
            </View>
            <Text className="mt-4 text-[16px] font-inter-medium text-[#7a8596]">Out Of Stock</Text>
            <Text className="mt-2 text-[24px] font-inter-bold text-[#171a21]">
          ...
          <Text className="text-[18px] font-inter-bold text-[#171a21]">
            Recent Movements
          </Text>
          ...
          {isStockMovementHistoryError ? (
            <Text className="text-[16px] font-inter-regular mt-4 text-[#7a8596]">Error loading recent movements</Text>
          ) : (
            ...
            { stockMovementHistory.length > 0 ? stockMovementHistory.map((item, index) => {
                ...
                <View key={item.id} className={index > 0 ? "mt-6" : ""}>
                  <View className="flex-row">
                    <View className="mr-4 items-center">
                      ...
                      >
                        <Text
                          className={`text-[18px] font-inter-semibold ${
                            isStockIn
                              ? "text-[#159b67]"
                              : isStockOut
                                ? "text-[#b54343]"
                    ...
                    <View className="flex-1">
                      <Text className="text-[15px] font-inter-semibold text-[#171a21]">
                      ...
                      <View className="mt-3 self-start rounded-md border border-[#d9e4d8] bg-[#f4f8f3] px-3 py-2">
                        <Text className="text-[14px] font-inter-medium text-[#535862]">{badgeText}</Text>
                      </View>
                    </View>
                  </View>
                </View>
              ...
              <Text className="text-[16px] font-inter-regular mt-4 text-[#7a8596]">No recent movements</Text>
            )}
          </View>
          )}
        </View>
